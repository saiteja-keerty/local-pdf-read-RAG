...
MODELS = {
    ...
    'nemotron-4b': {'repo':'bartowski/nvidia_Nemotron-Mini-4B-Instruct-GGUF','filename':'nvidia_Nemotron-Mini-4B-Instruct-Q4_K_M.gguf'},
...
if args.model:
    ...
    p = hf_hub_download(repo_id=m['repo'], filename=m['filename'])
    dest = OUT / m['filename']
    Path(p).replace(dest)
    print('Saved to', dest)
else:
